...
!   PSLQM3 parameters set below; all are integer(4), except fname is character(64):
...
!   TPPHIX3 parameters set below; all are integer(4):
...
!     lcx   Size of character(1) array chr1; default = 64 * int (ndp/(64*n) + 1).
...
parameter (ipal = 1, kd = 28, kp = 1, kq = 1, lcx =  64 * int (1 + ndp/(64*n)), &
  lmx = 131072, mpi = 8, nep1 = -ndp)

integer i, iq, i1, i2, j, j1, k, l1, m, nn, n1
real (8) d1, d2, second, tm, tm0, tm1, tm2, tm3
external second
type (mp_real) alpha, beta, eps, eps1, pi, qq, xx, yy
type (mp_realm) epsm
...
!   Uncomment this line for MPFUN20-Fort when precision > 12,600 digits.
...
tm2 = second ()
call pslqm3 (idb, nn, nwds, nwdsm, ndr, nrb, nrs, fname, eps, epsm, x, iq, r)
tm3 = second ()
...
theta1 = 2.d0 * t0
write (6, 1) k1
1 format ('theta1 complete: k1 =',i6)
return
end
...
theta2 = 2.d0 * t0
write (6, 1) k1
1 format ('theta2 complete: k1 =',i6)
return
end
...
write (6, 1) k1
1 format ('theta3 complete: k1 =',i6)
return
end
...
write (6, 1) k1
1 format ('theta4 complete: k1 =',i6)
return
end
...
!   David H. Bailey    16 Feb 2021

subroutine pslqm3 (idb, n, nwds, nwdsm, ndr, nrb, nrs, fname, eps, epsm, x, iq, r)
...
!     ndr   int(4)   Log10 of the min dynamic range of |y_i| at detection, typically 30.
...
!     nrs   int(4)   0: Restart file is not read or written; typically 0.
!                    1: Start new run, and periodically write restart file.
!                    2: Read restart file, run and periodically write restart file.
!                    The input restart file is overwritten, so make a backup copy.
!     fname char(64) Filename for restart; typically 'pslqm3.rst'. If more than 
!                    one job is run in same directory, use distinct filenames.
!     eps   mp_real  Tolerance for full precision detection. See tpslqm3 comments.  
!     epsm  mp_realm Tolerance for medium precision detection. See tpslqm3 comments.
...
!     ndrm  int(4)   Extra precision beyond wy dynamic range for MPM iterations;
!                    default = 25.
!     nsq   int(4)   Size of tables used in iterdp and itermpw; default = 8.
!     dreps real(8)  Tolerance for DP dynamic range check; default = 1d-10.
...
real (8) d1, d2, d3, d4, dreps, dplog10, rb, second, &
  tm0, tm1, times(6)
parameter (dreps = 1d-10)
...
type (mp_realm) bound, dynrange, dynrangem, epsm, epsm2, wa(n,n), &
  wb(n,n), wh(n,n), wsyq(n,nsq), wy(n), wn, w1, w2, w3
...
if (idb >= 2) then
  ...
  write (6, 3) it, d1, n1
3 format ('Iteration',i8,2x,'Min/max ratio of y =',f11.6,'e',i6)
endif
...
!   DP iteration was aborted. If so, then go perform MPM iterations.

w1 = dynrangem (n, nwdsm, wy)
if (idb >= 3) then
  call mpdecmd (w1, d1, n1)
  write (6, 6) it, d1, n1
6 format ('Iteration',i8,2x,'Min/max ratio of wy =',f11.6,'e',i6)
endif
...
!   Restart DP iterations and continue.
...
if (izmm == 0 .and. mod (it - its, ipm) == 0) then
  w1 = dynrangem (n, nwdsm, wy)
  call mpdecmd (w1, d1, n1)
  if (idb >= 2) write (6, 16) it, d1, n1
16 format ('Iteration',i8,2x,'Min/max ratio of wy =',f11.6,'e',i6)
...
integer i, n, n1, nwdsm
real (8) d1
type (mp_realm) dynrangem, t1, t2, t3
type (mp_realm) wy(n)
...
tmx1 = 1.d0 / epsm / mprealdm (1d20, nwdsm)
...
integer ix(n)
real (8) da(n,n), db(n,n), dx(n), dreps
